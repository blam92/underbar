(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if(iterator !== undefined) {
      array = _.map(array, function(num) {
      iterator(num);
      });
    }
    ...
    _.each(array, function(value, key, collection) {
      if(isSorted) {
        ...
        for(var i = 0; i < collection.length; i++) {
          if(value === collection[i] && uniqArray.indexOf(value) !== -1) {
            duplicateFlag = true;
          }
        }
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var memo;
    var changedArray = collection.slice();

    if(accumulator !== undefined) {
      memo = accumulator;
    } else {
      memo = collection[0];
      changedArray = changedArray.slice(1, changedArray.length);

    }
    

    _.each(changedArray, function(value, key, collection) {
